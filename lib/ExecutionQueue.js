...
ExecutionQueue.run = function (task) {
  return new TaskRunner(task);
};


function TaskRunner (task, synchronous) {
  this._task = task;
  this._synchronous = synchronous;
}

TaskRunner.prototype.on = function (queue) {
  var task = this._task,
      synchronous = this._synchronous;
  
  queue.run(task, synchronous);
  
  return this;
};

TaskRunner.prototype.synchronously = function () {
  return new TaskRunner(this._task, true);
};

TaskRunner.prototype.asynchronously = function () {
  return new TaskRunner(this._task, false);
};
